...
function isWinningTicket(arr) {
  ...
  for (let i = 0; i < arr.length; i++){
    let charFromNumber = String.fromCharCode
    (arr[i][1])
...
function toCamelCase(string) {
     ...
     return string.charAt(4).toUpperCase()
...
function gridTrip(arr, string) {
  ...
  while (ind < string.length){
    ...
    total[lookup[direction][0] = num * lookup[direction][1]]
  }
  return  total
}
/*-----------------------------------------------------------------
...
function addChecker(nums, int) {
  let result = false
  for( i = 0; i < nums.length -1; i ++){
    for(n = i + 1; n < nums.length; n++){
      if (nums[i] + nums[n] == int)
      return true
    }
  }
  return result
