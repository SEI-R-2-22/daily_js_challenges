...
function formatWithPadding(n, char, length) { 
  let answer = `${n}`
  const sam = length-answer.length
  for (let i=0; i<sam; i++) {
    answer = `${char}${answer}`
  }
  return answer
...
function mapArray(arr, cb) { 
  ...
  arr.forEach((element, index) => {
    return newArr.push(cb(element, index))
  })
  return newArr
}
/*-----------------------------------------------------------------
...
function reduceArray(arr, acc, value) {
  ...
  arr.forEach((element, index) => {
    accumulator = acc(accumulator, element, index)
  })
  return accumulator
 }
/*-----------------------------------------------------------------
...
function intersection(arr1, arr2) { 
  ...
  arr1.forEach((element) => {
    let index = arr3.indexOf(element)
    if (index>-1) {
      answer.push(arr3.splice(index, 1)[0])
    }
  })
  return answer
}
...
function balancedBrackets(string) { 
  ...
  for (let i=0; i<string.length; i++) {
    ...
    if ('([{'.includes(chars)) {
      ...
      if (!'() {} []'.includes(answer.pop() + chars)) return false
    }
  }
  return true
}
...
function toCamelCase(string) {
  return string.replace(/[_-]\w/g, function (match) {
    return match.charAt(i).toUpperCase()
  })
 }
/*-----------------------------------------------------------------
...
function addChecker(arr, n) { 
  ...
  for (let i=0; i<arr.length-1; i++) {
    for(let s=0+1; s<arr.length; s++) {
      if (arr[i] + arr[s] === n) return true
    }
  }
  return answer
}
