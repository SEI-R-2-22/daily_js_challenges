...
function balancedBrackets(string) {
  ...
  let newArr = []
  for (let i = 0; i < string.length; i++) {
    let brackets = string.charAt(i)
    if ('([{'.includes(brackets)) {
      newArr.push(brackets)
    } else {
      if (!'() {} []'.includes(newArr.pop() + brackets)) return false
    }
  }
  return true
}
...
function gridTrip(arr, string) {
  ...
  while (idx < string.length) {
    ...
    result[lookup[dir][0]] += newString * lookup[dir][1]
  }
  return result
}
/*-----------------------------------------------------------------
