...
let computeRemainder = (n1, n2) => {
  ...
  let quotient = num1 - Math.floor(num1/num2) * num2
    return quotient
}
/*-----------------------------------------------------------------
Challenge: 06-range
...
const charCount = (string) => {
  return [...string].reduce((char, count) => {
    char[count] = char[count] ? char[count] + 1 : 1; return char
  }, {});
}
/*-----------------------------------------------------------------
...
const formatWithPadding = (n, char, length) => {
  numStr = `${n}`
  if (numStr.length >= length) {
    return numStr
  } else {
    return numStr.padStart(length, char)
  }
}
...
const isPalindrome = (string) => {
  ...
  let newString = stringLower.replace(/\s+/g, '');
...
const hammingDistance = (str1, str2) => {
  ...
  let secStr = str2.split('')

  let strCompare = firstStr.filter((letter, index) => secStr[index] !== letter)
...
const mumble = (string) => {
  ...
  let mumbleStr = newStr.map((letter, index) => letter + letter.repeat(index))

  return mumbleStr.join('-')
}
/*-----------------------------------------------------------------
...
const fromPairs = (arr) => {
	const newArr = [arr]
  let obj = newArr.map(pair => Object.fromEntries(pair))
  return obj[0]
}
/*-----------------------------------------------------------------
...
const findHighestPriced = (arr) => {
  let highestPrice = arr.reduce((prev, current) => (+prev.price > +current.price) ? prev : current)
  return highestPrice
}
/*-----------------------------------------------------------------
...
const reduceArray = (arr, acc, value) => {
  let newVal = value
  arr.forEach((item, index) => {
    newVal = acc(newVal, item, index)
  }) 
  return newVal
}
/*-----------------------------------------------------------------
...
// I understand this with the exception of n = n / i. I don't know what it is for. I'm terrible at math. Source: https://stackoverflow.com/questions/39899072/how-can-i-find-the-prime-factors-of-an-integer-in-javascript
/*-----------------------------------------------------------------
...
const balancedBrackets = (string) => {
  while (string.length > 0) {
    let testStr = string.replace('{}', '').replace('[]', '').replace('()', '')
    if (string == testStr) {
      return false
    }
    string = testStr
...
// I found this one very interesting, considering how short it was compared to other solutions I researched. I've never seen so many .replace()'s chained together.
// Also, I wonder why they chose to break it the loop with line 657. At least, that's my understanding of what that line is. I tested it with 'break' and it passed, that's the only evidence I have.
//Source: https://codereview.stackexchange.com/questions/217275/check-whether-string-of-braces-brackets-and-parentheses-is-balanced
/*-----------------------------------------------------------------
...
const isWinningTicket = (arr) => {
  let winner = arr.every((ticket) => {
    return ticket[0].includes(String.fromCharCode(ticket[1]))
  })
  return winner
}
...
function getNumForIP(bits) {
  let totalBits = 0
  bits.split('.').reverse().forEach((item, index) => {
    totalBits += parseInt(item) * 256 ** index
  })
  return totalBits
}
/*-----------------------------------------------------------------
...
const toCamelCase = (string) => {
  return string.replace(/[^a-zA-Z0-9]+(.)/g, (match, char) => char.toUpperCase());
}
/*-----------------------------------------------------------------
...
function countTheBits(n) {
  ...
  let bits = [...numStr].filter(bit => bit == '1').length
  return bits
...
const gridTrip = (arr, string) => {
  let coords = [arr[0], arr[1]]
  let dirArr = string.split(/([0-9]+)/)
}

// I honestly had no clue how to do this. I couldn't get past a way to split the string after each integer, only found a way to split it into numbers and letters.
/*-----------------------------------------------------------------
...
const addChecker = (arr, n) => {
  let sum = arr.some((num, index) =>
    arr.slice(index + 1).includes(n - num)
  )
  return sum
}
/*-----------------------------------------------------------------
...
const totalTaskTime = (arr, n) => {
  let totalTime = arr.reduce((a, b) => a + b, 0)
}

//I didn't understand this one at all. I didn't know where to start. I even looked at the solution and didn't really understand it.
/*-----------------------------------------------------------------*/
module.exports = {
  sayHello,
  addOne,
