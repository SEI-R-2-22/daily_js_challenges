...
function range(n1, n2) {
  const arr = []
  const r = n2 - n1
  const msg = 'First argument must be less than second'
  if (n1 > n2) {
    return msg
  } else if (n1 === n2) {
    return []
...
function reverseUpcaseString(string) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    newString += string[length - i - 1].toUpperCase()
  }
  return newString
}
/*-----------------------------------------------------------------
...
function charCount(string) {
  ...
  for (let i = 0; i < len; i++) {
    let key = string[i]
    let value = 1
    !countString[key] ? (countString[key] = value) : (countString[key] += value)
  }
  return countString
}
...
function findHighestPriced(arr) {
  ...
  let highestPrice = 0
  arr.forEach((el) => {
    if (el.price > highestPrice) {
      highestPrice = el.price
      temp = el
    }
  })
  return temp
...
function intersection(arr1, arr2) {
  ...
  arr1.forEach((el) => {
    ...
    if (idx > -1) temp.push(...arr2.splice(idx, 1))
  })
  return temp
}
/*-----------------------------------------------------------------
...
function getNumForIP(bits) {
  const ipArr = bits.split('.')
  const multipler = [16777216, 65536, 256, 1]
  let bit = 0
  ipArr.forEach((el, idx) => {
    bit += parseInt(el) * multipler[idx]
  })
  return bit
}
/*-----------------------------------------------------------------
...
function gridTrip(arr, string) {
  ...
  for (let i = 0; i < string.length; i++) {
    if ('LRDU'.includes(string[i])) {
      direction = string[i]
      for (let j = i + 1; j < string.length; j++) {
        if (!'LRDU'.includes(string[j])) {
          move += string[j]
        } else break
      }
      if (direction === 'D') {
        console.log(direction + ' ' + move + ' ' + grid)
        grid[0] -= parseInt(move)
        console.log(grid)
      } else if (direction === 'U') {
        console.log(direction + ' ' + move + ' ' + grid)
        grid[0] += parseInt(move)
