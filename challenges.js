...
function range(first, second) {
  if (first > second) return "First argument must be less than second"
    else {
      let range = []
...
function charCount(string) {
  ...
  for (letter of string) {
    if (!countNum[letter]) countNum[letter] = 1;
    else countNum[letter]++;
  }
  return countNum;
}
/*-----------------------------------------------------------------
...
function flatten(arr) {
  ...
  arr.forEach((flat) => {
    ...
    if (Array.isArray(flat)) {
      flatArr = flatArr.concat(flatten(flat))
    } else {
      flatArr.push(flat)
    }
  })
