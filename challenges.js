...
function range(n1, n2) {
  let rangeArray = [];
  if (n1 <= n2) {
    for (i = n1; i < n2; i++) rangeArray.push(i);
    return rangeArray;
  } else {
...
function reverseUpcaseString(string) {
  ...
  let uppcaseNewString = newString.toUpperCase();
  return uppcaseNewString;
}
/*-----------------------------------------------------------------
...
function charCount(string) {
  ...
  for (i = 0; i < string.length; i++) {
    if (string[i] in charCountObject) {
      charCountObject[string[i]] += 1;
    } else {
      charCountObject[string[i]] = 1;
    }
  }
...
function intersection(a1, a2) {
  ...
  let _a2 = [...a2];
  a1.forEach((val) => {
    let index = _a2.indexOf(val);
    if (index > -1) result.push(_a2.splice(index, 1)[0]);
  });
  return result;
}
/*-----------------------------------------------------------------
...
function gridTrip(xyArr, moves) {
  ...
  while (index < moves.length) {
    ...
    index++;
    let numString = "";
    while ("0123456789".includes(moves[index]) && index < moves.length) {
      numString += moves[index];
      index++;
    }
