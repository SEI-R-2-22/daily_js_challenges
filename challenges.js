...
function sumNumbers(nums) {
  const sums = nums.reduce((a,b) => {return a + b}, 0)
  return sums
}
/*-----------------------------------------------------------------
...
function charCount(string) {
  ...
  for (i = 0; i < string.length; i++) {
    ...
    results[character] ? results[character]++ : results[character] = 1
  }
  return results
}
/*-----------------------------------------------------------------
...
function isPalindrome(string) {
  let result = string.toLowerCase().replace(/[^\w\s\']/g, "").replace(/\s/g, "")
...
function hammingDistance(str1, str2) {
  ...
  for (i = 0; i < str1.length; i++) {
    if ( str1.charAt(i) !== str2.charAt(i) ) hammCount++
  }
  return hammCount
}
/*-----------------------------------------------------------------
...
function mumble(string) {
  let results = ""
  for (i = 0; i < string.length; i++) {
    results += ((i || '') && '-') + string.charAt(i).repeat(i + 1)
  }
  return results
}
/*-----------------------------------------------------------------
...
function fromPairs(arr) {
  let pairs = arr.reduce((object, newArray) => {
...
function findHighestPriced(arr) {
  let highest = arr.reduce((x, y) => y.price > x.price ? y : x)
  return highest
}
/*-----------------------------------------------------------------
...
//I cheated and looked at the solution first for this one because I couldn't understand what it was asking of me. But once I understood, I did write it myself.
function mapArray(arr, cb) {
  let newArray = []
  arr.forEach(function (a, index) {
    newArray.push(cb(a, index))
  })
return newArray
}
/*-----------------------------------------------------------------
...
function flatten(arr) {
  ...
  arr.forEach( function (a) {
    if (Array.isArray(a)) {
      flatArray = flatArray.concat(flatten(a))
    } else {
      flatArray.push(a)
    }
  })
...
//had to cheat a lil with this one too, but i got it!
function primeFactors(n) {
  let factorTree =[]

  if (n < 2 || !Number.isInteger(n)) return factorTree
...
function intersection(arr1, arr2) {
  ...
  arr1.forEach((value) => {
    ...
    if (index > -1) final.push(copy2.splice(index, 1)[0])
  })
  return final
}
/*-----------------------------------------------------------------
...
function balancedBrackets(string) {
  // if (string.length % 2 ) return falsep
...
function isWinningTicket(arr) {
  let winningTicket = arr.every(function(a) {
        return a[0].includes(String.fromCharCode(a[1]))
      })
...
function toCamelCase(string) {
  let camelCase = string.replace(/[-_]\w/gm, function (a) {
