...
function computeRemainder(n1, n2) {
  return (n2 !== 0) ?  (n1 - ((Math.floor(n1/n2)) * n2)) : Infinity
}
/*-----------------------------------------------------------------
Challenge: 06-range
...
function reverseUpcaseString(string) {
    ...
    let resplit = string.split(``)
    for (let i=0; i<split.length; i++) {
      split[i] = resplit[split.length-1-i]
    }
...
function isPalindrome(string) {
  const fstr = string.toLowerCase()
  const sstr = fstr.split('')
  const tstr = sstr.filter((letter) => letter !== ' ')
  let pal = true
  for (i=0; i< tstr.length; i++){
    if (tstr[i] !== tstr[tstr.length - 1 - i]){
      pal = false
    }
  }
  return pal
...
const tstr = sstr.filter((brace) => brace !== ' ')
let bal = false
for (i=0; i< tstr.length; i++){
  for (j=0; j < tstr.length + i; j++){
    if (tstr[i] === '('){
      if (tstr[j] == ')' && (j > i) &&(j - i) % 2 !== 0){
        bal = true
      } 
    }
    else if (tstr[i] === '{'){
...
function isWinningTicket(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let code = String.fromCharCode(arr[i][1])
    if (arr[i][0].includes(code)) {
      isWin = true
    }
    else if (!arr[i][0].includes(code)){
      isWin = false
      return isWin
    }
  }
...
function getNumForIP(bits){
  let newString = bits.split('.')
  let sum = 0
  for (let i = 0; i < newString.length; i++){
    let val = parseInt(newString[i]) * (256 ** (3 - i))
    sum += val
  }
  return sum
}
