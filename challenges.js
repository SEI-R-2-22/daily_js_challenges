...
const computeRemainder = (n1, n2) => (n2 ===0) ? Infinity : n1 - (Math.floor(n1 / n2)) * n2
...
const reverseUpcaseString = (string) => string.split('').reverse().join('').toUpperCase()
...
const isPalindrome = (string)=> {
  ...
  let reverseString = string.split("").reverse().join("").replace(/\s+/g, "").toLowerCase()
  if(string.replace(/\s+/g, "").toLowerCase() === reverseString) {
    return true
  } else {
    return false
  }
...
const mumble = (string) => {
  let newStr = ''
  for (let i = 0; i < string.length; i++) {
    newStr += ((i || '') && '-') + string.charAt(i).repeat(i + 1)
  }
  return newStr
}
/*-----------------------------------------------------------------
...
const mapArray = (arr, cb) => {
    const newArr = []
    arr.forEach((e, i) => newArr.push(cb(e, i)))
    return newArr
}
/*-----------------------------------------------------------------
...
const isWinningTicket = (arr) => {
  for (let i = 0; i < arr.length; i++) {
    let char = String.fromCharCode(arr[i][1])
...
const getNumForIP = (bits) => {
  let num = 0
  const reverseBits = bits.split('.').reverse()
  reverseBits.forEach((bit, i) => num += parseInt(bit) * 256 ** i)
  return num
}
/*-----------------------------------------------------------------
...
const countTheBits = (n) => n.toString(2).split('').filter(e => e === '1').length
/*-----------------------------------------------------------------
