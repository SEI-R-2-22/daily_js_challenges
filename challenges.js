...
function mumble(string) {
  let char = "";
  for (let i = 0; i < string.length; i++) {
    char += ((i || "") && "-") + string.charAt(i).repeat(i + 1);
  }
  return char;
}
/*-----------------------------------------------------------------
...
function flatten(arr) {
  ...
  arr.forEach(function (elm) {
    flat = flat.concat(Array.isArray(elm) ? flatten(elm) : elm);
  });
  return flat;
}
/*-----------------------------------------------------------------
...
function gridTrip(arr, string) {
  ...
  while (num < string.length) {
    ...
    num++;
    let numString = "";
    while ("0123456789".includes(string[num]) && num < string.length) {
