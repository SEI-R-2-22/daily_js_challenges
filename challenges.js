...
function isPalindrome(string) {
  ...
  const formattedStringForward = string.toLowerCase().replace(/\s/g, '')
  const formattedStringReverse = string
    .split('')
    .reverse()
    .join('')
    .toLowerCase()
    .replace(/\s/g, '')
  return formattedStringForward === formattedStringReverse
}
...
function fromPairs(arr) {
  ...
  arr.forEach((nestedArr) => {
    object[nestedArr[0]] = nestedArr[1]
  })
  return object
}
/*-----------------------------------------------------------------
...
function mapArray(arr, cb) {
  ...
  arr.forEach((element, index) => {
    return newArray.push(cb(element, index))
  })
  return newArray
}
...
function intersection(arr1, arr2) {
  ...
  arr1.forEach((element) => {
    let index = arr2Copy.indexOf(element)
    if (index > -1) {
      resultArr.push(arr2Copy.splice(index, 1)[0])
    }
  })
  return resultArr
}
