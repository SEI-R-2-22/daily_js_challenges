...
function reverseUpcaseString(string) {
  return string.split('').reverse().map(function(string) {
...
function mumble(string) {
  let repeated = ''
  for (let i = 0; i < string.length; i++) {
    repeated += ((i || '') && '-') + string.charAt(i).repeat(i + 1)
  }
  return repeated;
}
/*-----------------------------------------------------------------
...
function reduceArray(arr, acc, value) {
  ...
  arr.forEach((element, indx) => {
    accumulator = acc( accumulator, element, indx)
  })
  return accumulator
}
/*-----------------------------------------------------------------
...
function intersection(arr1, arr2) {
  ...
  arr1.forEach((item) => {
    ...
    if (index > -1) {
      newArr.push(array2.splice(index, 1)[0])
    }
  })
  return newArr
}
...
function balancedBrackets(string) {
  ...
  let result = []
  for (let i = 0; i < string.length; i++) {
    let chars = string.charAt(i)
    if ('([{'.includes(chars)) {
      result.push(chars)
    } else {
    if (!'() {} []'.includes(result.pop() + chars)) return false
    }
  }
  return true
}
...
function isWinningTicket(array) {
  for (let i = 0; i < array.length; i++) {
    let newArr = String.fromCharCode(array[i][1])
    if (!array[i][0].includes(newArr)) {
      return false
    } else {
      return true
    }
...
function getNumForIP(bits) {
  let values = bits.split('.').reverse()
  let sum = 0
  values.forEach((val, index) => {
    sum += parseInt(val) * 256 ** index
  })
  return sum
}
/*-----------------------------------------------------------------
...
function toCamelCase(string) {
  let regex = /[_-]\w/g
   return string.replace(regex, match => match.charAt(1).toUpperCase())
}
/*-----------------------------------------------------------------
...
function gridTrip(arr, string) {
  ...
  const coords = {
    ...
    let numString = ''
      ...
      result[coords[direction][0]] += numString * coords[direction][1]
    }
    return result
}
/*-----------------------------------------------------------------
...
while (arr.length > n) {
  ...
  time += shortest
  threads = threads.map((task) => task - shortest).filter((task) => task)
