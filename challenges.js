...
function addTwoNumbers(num1, num2) {
  ...
  // } if (num1 + num2 === 0){
  //     return 0;
  //   } if (addTwoNumbers('')) { ////////NEED HELP WRITING OUT THE NAN THE OTHER ONES ARE CORRECT
  //     return NaN;
  //   }3
  // 
...
//   return reverse 
//I TESTED THE ABOVE IN CHROME AND IT WORKED BUT IT FAILED HERE SO I PASTED SOLUTIONS
...
// WAS RUNNING THIS AT FIRST AND IT KEPT RETURNING UNDEFINED BUT THE SOLUTIONS IS VERY SIMILAR 
//  let empty = '
// if(string < 3){
//   return empty
...
//Got  in stack overflow https://stackoverflow.com/questions/18619785/counting-frequency-of-characters-in-a-string-using-javascript
...
function formatWithPadding(n, char, length) {
  ...
  if(leng >= length){
    ...
    let padding = length - leng
    let repeat = char.repeat(padding)
    let newNum = repeat.concat(stri)
    return newNum
  }
}
formatWithPadding(1236, '*', 3)
...
function isPalindrome(string) {
    let spl = string.split('') 
      // return spl
    let backwards = spl.reverse('')
      // return backwards
    let turnString = backwards.join('');
    // return turnString
    let leng = string.length
    
    if(turnString === string || leng<=1 ){
        return true
      }else {
        return false
      }
...
function hammingDistance(str1, str2) {
  let stringOne = str1.length 
  let stringTwo = str2.length
...
for ( i = 0; i < string.length; i++) {
  ...
  aString += ((i || '') && '-') + string.charAt(i).repeat(i + 1)
}
return aString
} mumble('hello')
...
function flatten(arr) {
  ...
  arr.forEach((flat) => {
    ...
    if (Array.isArray(flat)) {
      anArr = anArr.concat(flatten(flat))
    } else {
      anArr.push(flat)
    }
  })
...
function primeFactors(n) {
  let factor = []
  if (n < 2 || !Number.isInteger(n)) return factor
  function isPrime(n) {
...
function intersection(arr1, arr2) {
  ...
  if (arr1.length && arr2.length) {
    arr1.length > arr2.length ? inter(arr2, arr1) : inter(arr1, arr2);
  }
...
function gridTrip(arr, string) {
  ...
  while (idx < string.length) {
    ...
    idx++
    let numString = ''
    while ('0123456789'.includes(string[idx]) && idx < string.length) {
      numString += string[idx]
      idx++
    }
...
function totalTaskTime(arr, n) {
 //I copied this one from solutions I didn't udnerstand it, I think I am tired haha
  lettime = 0,
    shortest,
    threads
